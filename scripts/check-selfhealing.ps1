...
foreach ($path in $installPaths) {
    if (Test-Path "$path\RGUI.exe") {
        ...
        if (Test-Path "$path\RGWorker.exe") {
            Write-Host "    ✓ RGWorker.exe exists" -ForegroundColor Green
        } else {
            Write-Host "    ✗ RGWorker.exe NOT FOUND" -ForegroundColor Red
        ...
        if (Test-Path "$path\RGService.exe") {
            Write-Host "    ✓ RGService.exe exists" -ForegroundColor Green
        } else {
            Write-Host "    ✗ RGService.exe NOT FOUND" -ForegroundColor Red
        }
        break
    }
}
